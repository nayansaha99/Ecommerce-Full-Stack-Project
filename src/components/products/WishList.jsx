...
const WishList = ({ setWishlistOpen, wishlistOpen, showNotif }) => {
    ...
    const addToWishlist = (product) => {
        ...
        showNotif(`"${product.name.slice(0, 28)}..." added to wishlist`);
    };
                                    ...
                                    <div key={item.id} className="flex gap-3 bg-slate-50 rounded-xl p-3 border border-slate-200">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mt-2">
                                                <button onClick={() => setWishItems(prev => prev.map(i => i.id === item.id ? { ...i, qty: Math.max(1, i.qty - 1) } : i))} className="w-7 h-7 rounded-lg bg-white border border-slate-200 text-slate-700 font-bold flex items-center justify-center hover:bg-slate-100 transition-colors text-lg leading-none">-</button>
                                                <span className="text-sm font-bold text-slate-800 w-5 text-center">{item.qty}</span>
                                                <button onClick={() => setWishItems(prev => prev.map(i => i.id === item.id ? { ...i, qty: i.qty + 1 } : i))} className="w-7 h-7 rounded-lg bg-white border border-slate-200 text-slate-700 font-bold flex items-center justify-center hover:bg-slate-100 transition-colors text-lg leading-none">+</button>
                                                <button onClick={() => setWishItems(prev => prev.filter(i => i.id !== item.id))} className="ml-auto p-1.5 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors">
                                                    <X className="w-4 h-4" />
                                                </button>
                                            </div>
                                        </div>
